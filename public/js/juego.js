...
// Extrae solo el primer nombre (antes de espacio o guión bajo)
...
socket.on("roundStarted", (data) => {
  if (drawerNameDisplay) drawerNameDisplay.textContent = primerNombre(data.drawerName);
...
socket.on("updateScoreboard", (players) => {
  ...
  players.forEach(p => {
    const nombre = primerNombre(p.username);
...
socket.on("gameEnded", (data) => {
  ...
  const tablaPodioHTML = podio.map((p, index) => `
    <div class="flex justify-between items-center bg-slate-800/60 p-3 rounded-xl border border-slate-700/50 my-2">
      <span class="font-bold text-white">${index + 1}. ${primerNombre(p.username)}</span>
      <span class="text-cyan-400 font-black">${p.score || 0} pts</span>
    </div>
  `).join("");

  // El mensaje del ganador también puede tener el username largo — reemplazarlo
  const ganadorTexto = data.ganador
    ? data.ganador.replace(/es (.+?) con/, (_, u) => `es ${primerNombre(u)} con`)
    : "";
  ...
  if (canvasArea) {
    canvasArea.innerHTML = `
      <div class="text-center p-8 bg-slate-900 border border-cyan-500/30 rounded-3xl max-w-md mx-auto shadow-2xl max-h-[500px] overflow-y-auto">
        ...
        <p class="text-xl font-bold text-green-400 mb-6">${ganadorTexto}</p>
        ...
        <button
          ...
          class="w-full bg-cyan-500 hover:bg-cyan-400 text-black font-black px-8 py-3 rounded-xl transition-all shadow-lg shadow-cyan-500/20"
        >
          Volver al Lobby
        </button>
      </div>
